package com.richard.bigdata.weblog.clickstream;
...
import com.richard.bigdata.weblog.mrbean.WebLogBean;
			...
			try {
				...
				for (int i = 0; i < beans.size(); i++) {
					...
					if (1 == beans.size()) {
						...
						// 设置默认停留时长为60s
						v.set(session+"\001"+key.toString()+"\001"+bean.getRemote_user() + "\001" + bean.getTime_local() + "\001" + bean.getRequest() 
						+ "\001" + step + "\001" + (60) + "\001" + bean.getHttp_referer() + "\001" + bean.getHttp_user_agent() + "\001" + bean.getBody_bytes_sent() + "\001"
								+ bean.getStatus());
					...
					if (i == beans.size() - 1) {
						// 设置默认停留市场为60s
						v.set(session+"\001"+key.toString()+"\001"+bean.getRemote_user() + "\001" + bean.getTime_local() + "\001" + bean.getRequest() + "\001" + step + 
					 "\001" + (60) + "\001" + bean.getHttp_referer() + "\001" + bean.getHttp_user_agent() + "\001" + bean.getBody_bytes_sent() + "\001" + bean.getStatus());
						context.write(NullWritable.get(), v);
					}
				}
...
//		FileOutputFormat.setOutputPath(job, new Path(args[1]));
		...
		FileInputFormat.setInputPaths(job, new Path("D:\\textcode\\web_clike_mr_log\\clickstreamPageView\\input"));
		FileOutputFormat.setOutputPath(job, new Path("D:\\textcode\\web_clike_mr_log\\clickstreamPageView\\output"));
